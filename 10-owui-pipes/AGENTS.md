...
- **Fallback & Orchestration** : Gère la création du payload de secours (`fallback_payload`). L'outil `agent_orchestration_tool` y est désormais injecté manuellement (hardcodé) avec une consigne stricte dans le `system_prompt` ("utilise obligatoirement l'outil..."), garantissant le maintien des capacités d'orchestration agentique même lorsque le système bascule sur le modèle de secours minimal.

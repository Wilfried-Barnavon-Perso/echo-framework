...
- **Sémantique ASGI** : Le serveur expose ses outils non pas en SSE classique, mais via le protocole natif MCP sur HTTP (Transport SSE/HTTP).
- **Middleware d'Interception** : Il intègre un middleware pur ASGI conçu pour intercepter silencieusement l'en-tête `x-openwebui-user-id`. Cette interception permet au serveur de propager l'identité de l'utilisateur à l'intérieur des appels d'outils, garantissant que l'accès aux données (ex: SQLite Vault) est restreint à l'utilisateur courant.

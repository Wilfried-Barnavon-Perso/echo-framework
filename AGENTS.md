...
- **Sécurité Périmétrique :** BunkerWeb (WAF) protégeant le WebSocket WebGPU et l'API IdP. Refonte des règles de reverse proxy pour l'API REST (séparation stricte des codes d'erreurs applicatifs `400, 401, 404` laissés passants de manière transparente vers le LLM vs les erreurs de couche WAF `403, 429, 500, 502` interceptées et formatées en JSON natif). Le rate limiting (`limit_req off`) est explicitement désactivé sur les fichiers statiques de l'Open WebUI pour empêcher les erreurs 500 SvelteKit (quota 429) causées par de multiples reconnexions WebSocket.
- **Régulation & Consolidation :** Optimisation SQLite (Vacuum/WAL) et sauvegardes à chaud. Introduction du script automatisé `clean-echo.sh`. Le script d'installation centralise désormais l'**Autosafety Docker** : politique de logs stricte (max 10 Mo) et cron de nettoyage. Il implémente aussi un verrou mensuel (throttle de 30 jours, `.last_base_pull`) sur le pull de l'image `alpine:latest` pour préserver la bande passante. Le dashboard `server.py` permet d'invoquer manuellement une purge profonde (Cache APT, build cache, images orphelines) pour éradiquer tout risque de saturation disque. Le script de synchronisation (`sync-echo.sh`) intègre un nettoyage conditionnel intelligent des caractères Windows (CRLF/BOM) via `sed`, préservant l'horodatage des fichiers sains et donc le cache de compilation Docker BuildKit.
...
- **Encodage Strict (UTF-8 LF) :** Tous les fichiers de configuration système (`Dockerfile`, `requirements.txt`, `.sh`, `.py`, etc.) DOIVENT être générés et modifiés par l'agent au format UTF-8 avec des fins de ligne Unix (LF). L'utilisation de retours chariots Windows (CRLF) est formellement proscrite sous peine de destruction silencieuse du cache BuildKit de Docker.

---
---
---
*Document de référence pour l'agent ECHO - Version de Stack Actuelle : 5.209.48*

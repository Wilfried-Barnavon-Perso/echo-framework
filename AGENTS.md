...
- **Suture Bit-Perfect des Métadonnées :** Reconstruction de l'historique via SQLite (`message_shadows`) pour une continuité absolue. Garantit une reprise de session identique au bit près via l'ID de message et le timestamp (Verrou de Version). Suivi de la branche active et état de session garanti par un hash cumulatif (Cumulative Hash). La persistance des signatures cognitives (`cognitive_signatures`) intègre un **Tri Chronologique Déterministe** (`ORDER BY updated_at DESC LIMIT 1`) associé à un index sur `message_id` pour préserver de manière infaillible la dernière signature lors des cascades cognitives itératives (ex: boucle d'outils intra-requête ou MAX_TOKENS). L'injection de ces signatures (`thoughtSignature`) dans le payload respecte strictement la norme des appels parallèles Gemini 3+ en s'attachant exclusivement au premier `functionCall`.
...
- **ECHO Auth (SSO & MFA) :** IdP autonome gérant l'authentification forte (TOTP) couplé à BunkerWeb. Prise en charge des comptes locaux et OAuth2 (avec un mécanisme de **Fallback API Keys AI Studio** intégré si OAuth2 échoue). Intègre l'invalidation proactive de la session interne d'Open WebUI lors de la déconnexion globale du SSO pour éviter les collisions. La suppression d'une identité entraîne la **purge atomique totale** sur toutes les bases SQLite associées (chat, identity, MCP, N8N).
...
- **Sécurité Périmétrique :** BunkerWeb (WAF) protégeant le WebSocket WebGPU et l'API IdP. Refonte des règles de reverse proxy pour l'API REST (séparation stricte des codes d'erreurs applicatifs `400, 401, 404` laissés passants de manière transparente vers le LLM vs les erreurs de couche WAF `403, 429, 500, 502` interceptées et formatées en JSON natif). Le Rate Limiting global a été massivement assoupli (Burst 1000, Rate 300r/s) pour absorber les reconnexions WebSocket erratiques de la SPA SvelteKit sans bloquer l'interface. **Important (Multisite & Autoconf)** : Les configurations globales de BunkerWeb (ex: `DISABLE_DEFAULT_SERVER`, `AUTO_LETS_ENCRYPT`) DOIVENT impérativement être définies dans une ancre YAML partagée (`x-bw-config`) pour être injectées dans `bw-autoconf`. Il est **strictement interdit** de les définir via des labels (ex: `bunkerweb.AUTO_LETS_ENCRYPT=yes`) sur des conteneurs backend dynamiques (comme `echo-open-webui`). Lors d'un redémarrage (`docker restart`), Autoconf purge les labels du conteneur en cours d'arrêt, effaçant ainsi la variable globale de sa base de données. Cela entraîne la désactivation de Let's Encrypt, la génération d'un bloc catch-all `www.example.com`, des pertes de certificats (Rate Limit Let's Encrypt), et des conflits `server_name` Nginx.
- **Régulation & Consolidation :** Optimisation SQLite (Vacuum/WAL) et sauvegardes à chaud. Introduction du script automatisé `clean-echo.sh`. Le script d'installation centralise désormais l'**Autosafety Docker** : politique de logs stricte (max 10 Mo) et cron de nettoyage. Il implémente aussi un verrou mensuel (throttle de 30 jours, `.last_base_pull`) sur le pull des images `alpine:latest` et `n8nio/n8n:latest` pour préserver la bande passante tout en gardant le moteur à jour. Le dashboard `server.py` permet d'invoquer manuellement une purge profonde (Cache APT, build cache, images orphelines) pour éradiquer tout risque de saturation disque. Le script de synchronisation (`sync-echo.sh`) intègre un nettoyage conditionnel intelligent des caractères Windows (CRLF/BOM) via `sed`, préservant l'horodatage des fichiers sains et donc le cache de compilation Docker BuildKit.
...
- **UI HUD & Sécurité DOM (`echo_ui.py` & `echo_events.py`) :** Le moteur de rendu implémente un système natif **"OWUI Tools"** avec des optimisations 100% Mobile Natives (unités dvh, anti-zoom iOS, touch targets 44px, anti-scroll du body). La couche de communication UI (`echo_events.py`) intègre le protocole **UCTP (Universal Chunked Transfer Protocol)** qui fragmente dynamiquement les payloads JS massifs en chunks de 256 Ko (contournant les limites drastiques des WAF/WebSockets) avec injection d'un HUD de transfert non-bloquant (Spinner & Progress Bar). Le système injecte également des modales asynchrones interactives (ex: `window.echoCustomPrompt` enrichies par des boutons 'pills' pour les options) respectant le mode sombre/clair pour ne jamais bloquer l'Event Loop, et sécurise les rendus HTML via `window.echoSanitizeHTML`.
...
- **Propagation d'Identité Asynchrone :** L'identité d'un sous-agent (`sub_sid`, `is_subagent`) contourne délibérément l'encapsulation stricte (falsification de signature) d'Open WebUI grâce à l'utilisation native de `contextvars` via `ECHO_SUBAGENT_CONTEXT`.
...
- **Encodage Strict (UTF-8 LF) :** Tous les fichiers de configuration système (`Dockerfile`, `requirements.txt`, `.sh`, `.py`, etc.) DOIVENT être générés et modifiés par l'agent au format UTF-8 avec des fins de ligne Unix (LF). L'utilisation de retours chariots Windows (CRLF) est formellement proscrite sous peine de destruction silencieuse du cache BuildKit de Docker. Le fichier `.gitattributes` force d'ailleurs cet encodage LF sur le `Dockerfile` et les `requirements.txt`.

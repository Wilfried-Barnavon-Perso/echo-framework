...
- **Démembrement de l'ancien `echo_utils.py`** : Le framework a subi une refonte modulaire majeure. Le cœur utilitaire massif a été éclaté en composants ultra-spécialisés :
  ...
  - **`echo_http.py`**, **`echo_events.py`**, **`echo_logger.py`**, **`echo_paths.py`**, **`echo_prompts.py`** : Briques fondamentales gérant respectivement les requêtes HTTP, évènements OWUI, log, chemins absolus, et gabarits de prompts. **`echo_events.py`** (façade UI) intègre nativement le protocole **UCTP (Universal Chunked Transfer Protocol)** (`emit_execute` & `call_execute`) qui fragmente les volumineux scripts JS en blocs de 256 Ko couplés à un HUD visuel de transfert, garantissant une étanchéité parfaite face aux limites réseau (WebSocket/WAF). Le fichier `echo_prompts.py` inclut quant à lui les domaines `SYS_RAG_DISTILL` et `SYS_ORCHESTRATOR_N8N_GRAPHER`.
